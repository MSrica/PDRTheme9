...
```{r rinex function calculation}
alphaBetaCalculation <- function(rinexLine){
  ionRow  <- str_trim(gsub('\\s+',' ', rinexLine))
  ionData <- str_split(ionRow, ' ')[[1]][1:4]
  ion     <- c(
    as.double(sub('D', 'e', ionData[1])),
    as.double(sub('D', 'e', ionData[2])),
    as.double(sub('D', 'e', ionData[3])),
    as.double(sub('D', 'e', ionData[4]))
  )
  return(ion)
}
```
...
ionisation <- data.frame(
  alphaIon = alphaBetaCalculation(rinexData[4]),
  betaIon  = alphaBetaCalculation(rinexData[5])
)
...
for (rowIndex in 23:nrow(sp3Data)-1) {
  ...
  if (length(row) == 7) {  
    ...
    gpsTime  <- (strtoi(row[5])*60 + strtoi(row[6]))*60
    next
  }
...
rm(rowIndex, row, data, dateTime, gpsTime)
```
...
for(gpsName in names(sp3DataFormatted)){
  ...
  for(gpsIndex in 1:nrow(sp3DataFormatted[[gpsName]])){
    ...
    distanceFormatted <- substr(str_interp('${ionDist$distance}${emptySpaces}'), 0, 20)
